...
is($user03->username, 'testaccount02', 'user03 username matches user02 username');
ok($user03->update({ username => 'testaccount03'}), 'Update user03 name');
ok($user03 = $schema->resultset('User')->find($user03->id), 'Check database for user03 update');
ok($user02 = $schema->resultset('User')->find($user02->id), 'Check database for user02 update');
...
$user03 = $schema->resultset('User')->search({username => 'testaccount03'})->single;
my $user04 = $schema->resultset('User')->update_or_create($account04_details);
is($user04->username, $account04_details->{username}, "Account has correct username");
