...
$test01->get_ok( "/login?username=test01&password=mypass", "Login test01" );
$test02->get_ok( "/login?username=test02&password=mypass", "Login test01" );
$test03->get_ok( "/login?username=test03&password=mypass", "Login test01" );

$test01->get_ok(
    "/databases/id/" . $db->db_id . "/edit_description",
...
my $sch_rs = $connection->resultset('CSchema');
my $sch    = $sch_rs->first;

$test01->get_ok(
    "/schemas/id/" . $sch->sch_id . "/" . $sch->db_id . "/edit_current",
...
$test01->get_ok(
    "/schemas/id/" . $sch->sch_id . "/" . $sch->db_id . "/edit_description",
...
my $tbl_rs = $connection->resultset('CTable');
my $tbl    = $tbl_rs->first;

$test01->get_ok(
    "/tables/id/" . $tbl->tbl_id . "/edit_current",
...
$test01->get_ok(
    "/tables/id/" . $tbl->tbl_id . "/edit_description",
...
SKIP: {
    ...
    $test01->get_ok( "/tasks/id/9/edit", "test01 request to tasks edit" );
    $test01->content_contains( 'Permission Denied', "test01 Denied task edit" );

    $test02->get_ok( "/tasks/id/9/edit", "test02 request to tasks edit" );
    $test02->content_contains( 'Task', "test02 Allowed task edit" );
    $test02->content_lacks( 'Permission Denied',
        "test02 Not denied task edit" );

    $test01->get_ok( "/datasets/id/2/edit", "test01 request to datasets edit" );
    $test01->content_contains( 'Permission Denied',
        "test01 Denied datasets edit" );

    $test02->get_ok( "/datasets/id/2/edit", "test02 request to datasets edit" );
    $test02->content_contains( 'Dataset Name', "test02 Allowed datasets edit" );

    $test03->get_ok( "/datasets/id/2/edit", "test03 request to datasets edit" );
    $test03->content_contains( 'Dataset Name', "test03 Allowed datasets edit" );

    $test01->get_ok( "/users/id/2/edit", "test01 request to users edit" );
    $test01->content_contains( 'Permission Denied',
        "test01 Denied users edit" );

    $test02->get_ok( "/users/id/2/edit", "test02 request to users edit" );
    $test02->content_contains( 'Permission Denied',
        "test02 Denied users edit" );

    $test03->get_ok( "/users/id/2/edit", "test03 request to users edit" );
    $test03->content_contains( 'Editing User', "test03 Allowed users edit" );
}
