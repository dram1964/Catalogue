...
ok(my $minimal_record = Catalogue::Importer::Database->new(
	...
	column_name => 'Column1'), "New record created with required-only fields");

isa_ok($minimal_record, 'Catalogue::Importer::Database');
ok($minimal_record->server_name($values[0]), 'inherits server_name Mutator');
ok($minimal_record->database_name($values[0]), 'inherits database_name Mutator');
ok($minimal_record->schema_name($values[0]), 'inherits schema_name Mutator');
ok($minimal_record->table_name($values[0]), 'inherits table_name Mutator');
ok($minimal_record->column_name($values[0]), 'inherits column_name Mutator');
...
undef($@);
eval {my $db1 = Catalogue::Importer::Database->new(server_name => 'TestServer');
}; 
...
ok($@, 	"Can't create database with just a server, database, schema, and table name");
SKIP: {
