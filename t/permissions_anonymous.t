...
my $allowed_anon = {
    ...
    "/datasets/id/"
      . $dataset->id
      . "/explore_json" => "Showing Dataset information",
...
for ( @{$user_logon_required} ) {
    ...
    $anon_ua->content_contains( "Please enter login details",
        "Logon required for $url_action" );

}

my $permission_denied =
  [ '/datasets/add', '/datasets/id/' . $dataset->id . '/edit', ];

for ( @{$permission_denied} ) {
    my $url_action = $_;
    $anon_ua->get($url_action);
    $anon_ua->content_contains( "Permission Denied",
        "Permission Denied for $url_action" );
}
...
SKIP: {
    ...
    my $user_login_required = {
        ...
        '/applications/search?search=cardiology' =>
          'Applications matching search term <strong>%cardiology%</strong>',
    ...
    my $requestor_login_required = {
        ...
        '/datarequest/id/'
          . $datarequest->id
          . '/review' => 'Reviewing Data Request ID: ',
    };
    ...
    for ( keys %$user_login_required ) {
        ...
        $anon_ua->get($url_action);
        $anon_ua->content_contains( "Please enter login details",
            "Anonymous denied " . $url_action );
        $anon_ua->content_lacks(
            $user_login_required->{$url_action},
            $url_action . " blocked for Anonymous"
        );
        $user->get($url_action);
        $user->content_contains(
            $user_login_required->{$url_action},
            $url_action . " allowed for User"
        );
        $curator->get($url_action);
        $curator->content_contains(
            $user_login_required->{$url_action},
            $url_action . " allowed for Curator"
        );
        $admin->get($url_action);
        $admin->content_contains(
            $user_login_required->{$url_action},
            $url_action . " allowed for Admin"
        );
    }
    ...
    for ( keys %$curator_login_required ) {
        ...
        $anon_ua->content_contains( "Please enter login details",
            "Anonymous denied " . $url_action );
        $anon_ua->content_lacks(
            $curator_login_required->{$url_action},
            "Requested page blocked for Anonymous"
        );
        $user->get($url_action);
        $user->content_contains( "Permission Denied",
            "User denied " . $url_action );
        $user->content_lacks(
            $curator_login_required->{$url_action},
            "Requested page blocked for User"
        );
    }
    ...
    for ( keys %$allowed_with_permission ) {
        ...
        $anon_ua->get($url_action);
        $anon_ua->content_contains( "Permission Denied",
            "Anonymous denied by permissions " . $url_action );
    }
    ...
    $user->content_contains( "Permission Denied",
        "test01 denied registration/list" );
    $user->get("/registration/ng_new");
    $user->content_contains( "You are already logged-in",
        "test01 denied registration/ng_new" );

    $user->get("/datarequest/list");
    $user->content_contains( "Permission Denied",
        "test01 denied datarequest/list" );
    $user->get("/tasks/list");
    ...
    $curator->content_contains( "Permission Denied",
        "test02 denied users/list" );
    $curator->get("/users/add");
    $curator->content_contains( "Permission Denied",
        "test02 denied users/add" );
    $curator->get("/users/search?search=example");
    $curator->content_contains( "Permission Denied",
        "test02 denied users/search" );
    $curator->get("/users/id/2/edit");
    $curator->content_contains( "Permission Denied",
        "test02 denied user/edit" );
    $curator->get("/users/id/2/delete");
    $curator->content_contains( "Permission Denied",
        "test02 denied user/delete" );
    ...
    $admin->content_contains( "Can't delete own account",
        "test03 denied delete self" );
}
