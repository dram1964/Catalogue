...
note('Testing that Controller Search actions require Logged-On User');
for my $controller ( keys %$controller_message ) {
    $ua2->get_ok( '/logout', 'Logout via URL' );
    $ua2->get_ok( "/$controller/search",
        "Request /$controller/search for empty user" );
    ...
    $ua2->content_contains(
        ...
        "Redirect to /$controller/search after login"
    );
}
